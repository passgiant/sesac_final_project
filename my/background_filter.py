...
#             text="A festive Christmas party scene with people wearing cozy sweaters, holding hot cocoa, and exchanging gifts under colorful Christmas lights. A Christmas tree adorned with golden ribbons and ornaments.",
...
def main(input_image, prompt, *func_args, **func_kwargs):
    """
    Main function to process an image (NumPy array) with a prompt.

    Args:
        input_image (np.ndarray): NumPy array representing the input image.
        prompt (str): The prompt text to use for processing.
        *func_args: Additional positional arguments.
        **func_kwargs: Additional keyword arguments.
    ...
    with torch.inference_mode(), ctx:
        # Load image node
        loadimage = NODE_CLASS_MAPPINGS["LoadImage"]()
        loadimage_1 = loadimage.load_image(image=pil_image)  # PIL 이미지 전달
        
        # print(f"LoadImage result: {loadimage_1}")
        # print(f"Image Tensor Shape: {loadimage_1[0].shape}, Mask Tensor Shape: {loadimage_1[1].shape}")
        ...
        # print(f"InspyrenetRembg result: {inspyrenetrembg_3}")
        ...
        # print(f"SplitImageWithAlpha result: {splitimagewithalpha_16}")

        # VAE encoding for inpainting
        vaeencodeforinpaint = NODE_CLASS_MAPPINGS["VAEEncodeForInpaint"]()
        vaeencodeforinpaint_17 = vaeencodeforinpaint.encode(
            grow_mask_by=6,
            pixels=get_value_at_index(loadimage_1, 0),
        ...
        # print(f"VAEEncodeForInpaint result: {vaeencodeforinpaint_17}")
        ...
        for q in range(args.queue_size):
            ...
            # print(f"KSampler result: {ksampler_21}")
            ...
            # print(f"VAEDecode result: {vaedecode_22}")

            # Convert output to NumPy array
            output_image = get_value_at_index(vaedecode_22, 0)
            
            # print(f"Before processing: Shape: {output_image.shape}, Dtype: {output_image.dtype}")

            # 텐서일 경우 NumPy 배열로 변환
            if isinstance(output_image, torch.Tensor):
                output_image = output_image.squeeze(0).cpu().numpy()  # 배치 차원 제거 및 NumPy 변환
                # print(f"After squeeze and NumPy conversion: Shape: {output_image.shape}, Dtype: {output_image.dtype}")

            # NumPy 배열 값 조정: [0, 1] → [0, 255], 데이터 타입 → np.uint8
            output_image = (output_image * 255).clip(0, 255).astype(np.uint8)
            # print(f"After scaling: Shape: {output_image.shape}, Dtype: {output_image.dtype}, Min: {output_image.min()}, Max: {output_image.max()}")
            ...
            try:
                ...
                # print(f"Failed to convert output to PIL: {e}")
