...
def main(image1, image2, image3, *func_args, **func_kwargs):
    global args, _custom_nodes_imported, _custom_path_added
    
    # print(image1)
    # print(image2)
    
    pil_image1 = Image.open(image1)  # 파일 경로를 통해 이미지 읽기
    image1_array = np.array(pil_image1)
    
    pil_acc_image = Image.fromarray(image1_array)
    pil_image = Image.fromarray(image2['background']) # background(원본 이미지), layers(칠한 부분?), composite(최종 합성 결과)
    
    # layers = image2['composite'] # 마스킹 색깔이 흰색일 때
    layers = image3 # 마스킹 이미지를 직접 집어넣기
    pil_mask_image = Image.fromarray(layers)
    ...
    with torch.inference_mode(), ctx:
        ...
        loadimage_422 = loadimage.load_image(image=pil_acc_image) # "christmas_headband_3.jpg"

        loadimage_590 = loadimage.load_image(image=pil_image) # "586560_719472_347.jpg"
        # print(loadimage_590)
        loadimage_0 = loadimage.load_image(image=pil_mask_image)
        ori_image, _ = loadimage_590
        mask_image, _ = loadimage_0
        loadimage_590 = (ori_image, mask_image)
        ...
        for q in range(args.queue_size):
            ...
            output_image = get_value_at_index(imagecompositemasked_676, 0)
            ...
            try:
                ...
                pil_image = None  # 에러 발생 시 None 반환
...
# # Wrapper function to use the main function with Gradio
# def process_images(image1: Image.Image, image2: dict):
#     """
#     This function processes two input images using the accessory_filter logic.

#     Args:
#         image1 (Image.Image): First input image.
#         image2 (dict): Second input image as a dict from Gradio ImageEditor.

#     Returns:
#         Image.Image: Processed output image.
#     """
#     # Convert the input images to paths or in-memory objects if necessary
#     image1_path = "input1.jpg"
#     image2_path = "input2.jpg"

#     # Save the first image
#     image1.save(image1_path)

#     # Extract and save the second image from the dict
#     try:
#         # Check if the "image" key exists in the returned dict
#         if "image" in image2:
#             # Extract the image as bytes and convert it to PIL.Image
#             edited_image = Image.open(io.BytesIO(image2["image"]))
#             edited_image.save(image2_path)
#         else:
#             raise ValueError("ImageEditor did not return an image.")
#     except Exception as e:
#         raise ValueError(f"Error processing ImageEditor data: {str(e)}")

#     # Update the global PROMPT_DATA with input image paths
#     PROMPT_DATA["422"]["inputs"]["image"] = image1_path
#     PROMPT_DATA["590"]["inputs"]["image"] = image2_path

#     # Run the main function and capture results
#     results = main()
#     output_image_path = results["filename_prefix"] + ".png"
...
with gr.Blocks() as demo:
    gr.Markdown("## Accessory Filter")
    with gr.Row():
        image1_input = gr.Image(label="Input Image 1", type="numpy")
        image2_input = gr.ImageEditor(label="Input Image 2 (Editable)", type="numpy", height=800)
