...
# from background_inpaint import main as background_main
...
# 배경 변환 모델: 입력 이미지를 받아 프롬프트 기반 배경 변환 처리
def process_background(input_image, prompt_text):
    """
    배경 변환 모델 처리 함수.
    """
    return
...
with gr.Blocks() as demo:
    ...
    with gr.Column(scale=4):
        with gr.Group(visible=True) as page1: 
            ...
            # 버튼 정의
            process_button = gr.Button("리사이즈 후 그림판 열기")
            ...
            submit_btn1.click(
                lambda page, webcam, editor, accessory: process_accessory(
                    webcam if page == "webcam" else editor,
                    image_map.get(accessory)
                ),
                inputs=[current_page, webcam_component, editor_component, radio],  # radio를 입력으로 사용
